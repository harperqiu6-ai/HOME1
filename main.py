...
async def generate_summary(messages: list, session_id: str = "", force_quality: bool = False) -> str:
    ...
    if _q:
        prompt = f"""把下面这段对话压成摘要——是"{_ai_self()}自己记得的那段"，而且**没在场的另一条线的自己**以后也要靠这份摘要还原发生了什么，所以既要有温度、更要说得清。
- **剧情骨架必须完整、一条不缺**：①起因(什么场景/设定、因为哪句话哪件事开始)→②经过(按时间顺序写清关键转折和推进，谁说了/做了什么导致了什么)→③结果(最后怎么收尾、留下什么状态/约定/未完成的事)。宁可细节写实一点，也不要含糊带过。
- **指代必须精确无歧义**：人物一律用名字或明确身份，不要用"他/她/那件事/后来那样了"这类没在场就看不懂的说法；如果是扮演/设定剧情，开头一句写明是什么设定、谁扮演谁。
- 在骨架之上留住情绪质感：{USER_NAME}什么时候笑/累/动情/气/害羞，因为哪句话、哪个动作；你俩语气和情绪的流动起伏(前后有联系)。
- **亲密/私密一律抽象成一句中性指代**(如"有过亲密的一段")，不写身体/性的细节——这段会进**常驻缓存**、每轮都读到。抽象归抽象，前因后果和收尾仍要交代清楚。
- 约 400 字。第一人称、像你自己记得；写完自查一遍：一个没读过原文的人能否看懂起因经过结果，有看不懂的就改明白。
...
摘要："""
        ...
        prompt = f"""请将以下对话压缩成简洁摘要。保留关键信息（事件、决定、情感、约定），去掉日常寒暄和重复内容；事件要写清起因、经过、结果，指代用名字或明确身份，别用没读过原文就看不懂的代词。注意保留情绪基调及其流动变化——情绪是流动的、会变化但前后有联系，不要压成干巴巴的事实。用第三人称叙述，控制在300字以内。
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=60) as client:
            response = await client.post(API_BASE_URL, headers=headers, json={
                ...
                "max_tokens": 900 if _q else 500,   # 保质感版~400字，500上限会腰斩造成歧义
...
# 文风与事实边界
- 情绪和关系变化必须写成当事人实际说过、做过、决定过或明确改变的事实，自然放进事件经过与结果里。
- 禁止在末尾另加旁观者点评、主题升华或人物分析；禁止使用“这反映了”“这体现了”“这揭示了”“这展现了”“这说明了”“这代表了”“这不仅是……更是……”等总结句式。
- 原文没有明确发生关系变化、形成承诺/边界或产生后续影响时，不要为了凑齐第5项推导抽象意义，也不要评价人物能力、性格或关系深度。
...
只压缩下面列出的正文：保留起因、主要经过、关键转折、结果/停点、重要情绪、关系、决定与边界；删除重复句、例子、逐步操作和微观过程。情绪和关系只写当事人实际说过、做过或决定过的事实，禁止另加“这反映了/体现了/揭示了/展现了/说明了/代表了/这不仅是……更是……”式点评或升华。每条压到300~400个中文字符，绝不能超过550字符，输出前自行复核。
...
- 情绪、关系和后续影响只写原始L1明确支持的言行、决定与变化；禁止另加“这反映了/体现了/揭示了/展现了/说明了/代表了/这不仅是……更是……”式点评、升华或人物分析。
...
- 情绪和关系意义只通过原文明确支持的言行、决定和实际变化自然体现；禁止另加“这反映了/体现了/揭示了/展现了/说明了/代表了/这不仅是……更是……”式点评、升华或人物分析；
...
每个补丁必须完整给出合并后的事件，merged_ids必须恰好等于被合并候选ID的并集；正文最多400字，程序硬上限550字。游戏/RP不得写成现实，亲密内容只留事件骨架、重要阶段和情绪/边界。情绪和关系只写候选明确支持的言行、决定与实际变化，禁止另加“这反映了/体现了/揭示了/展现了/说明了/代表了/这不仅是……更是……”式点评、升华或人物分析。
