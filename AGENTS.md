...
- **生成**：`generate_image()` 调 `{base}/images/generations`（硅基流动兼容格式），模型 `IMAGE_GEN_MODEL`（默认 `Kwai-Kolors/Kolors`）。key/base **默认复用 EMBEDDING_API_KEY / EMBEDDING_BASE_URL**（同为硅基流动，零新增密钥）；可用 `IMAGE_GEN_API_KEY/IMAGE_GEN_BASE_URL` 单独覆盖。开关 `IMAGE_GEN_ENABLED`（默认开）。均已注册 DB 覆盖 + `/api/settings` 热更新。
...
- **同图去重**：`save_image_memory` 按 md5 去重，重复画同一张时 `photo_id` 为 None，回一句"相册里已经有啦"。
